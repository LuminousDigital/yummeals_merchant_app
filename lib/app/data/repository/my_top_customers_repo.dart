...
class MyTopCustomersRepo {
  ...
  static Future<TopCustomersModel> getTopCustomers() async {
    var response;
    var dio = Dio(
      await getBasseOptionsWithToken(),
    );
    String url = APIList.topCustomers.toString();
    try {
      response = await dio.get(url);
      if (response.statusCode == 200) {
        return TopCustomersModel.fromJson(response.data);
      }
    } catch (e) {
      print(e);
    }
